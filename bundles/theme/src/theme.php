...
abstract class Theme {
	...
	public function render($page, $data = null)
	{
		...
		if (isset($view))
		{
			...
			// Start Tum Change to public view [@ 8.5.2013] ------------->
			
			// Old
			//$view->nest('theme_content', $page, $set_data);
			
			// New
			$path_page = $theme_path_absolute. "/". $page .EXT;

			if (!file_exists($path_page))
			{
				$path_page = $base_path . $this->_theme_path . DS . $this->_default_layout. "/". $page .EXT;
			}

			$view->nest('theme_content', "path: " . $path_page ,$set_data);
			// End change ----------------------------------------------->
	...
	{
		if (isset($this->_theme_partials['partials']) and (is_array($this->_theme_partials['partials'])))
		{
